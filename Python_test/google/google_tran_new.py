...
def translate2(file1, file2, srclang, tranlang, sindex):
    ...
    for line in lines:
        line = line.strip('\n')
        # calling -> proxy
        # iplist = ['101.132.142.124:8080','124.243.226.18:8888','101.6.130.67:8123']
        ...
        #transinfo = eval(ss.get(url % (srclang, tranlang, getTK(line), line), headers = headers, proxies=proxies, timeout = 5).text.replace('null', 'None').replace('true', 'True').replace('false', 'False'))
